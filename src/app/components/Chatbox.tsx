import { useCallback, useEffect, useState } from "react";
import {
  Avatar,
  Box,
  Divider,
  IconButton,
  styled,
  TextField,
  useTheme,
} from "@mui/material";
import { Attachment, Clear, TagFaces } from "@mui/icons-material";
import ScrollBar from "react-perfect-scrollbar";
import { H5, H6, Span } from "./Typography";
import { ChatAvatar } from "../components";
import { convertHexToRGB } from "../utils/utils";
import EmojiPicker from "emoji-picker-react";

// STYLED COMPONENTS
const ChatContainer = styled("div")({
  height: "100%",
  display: "flex",
  flexDirection: "column",
  background: "#fff",
});

const StyledScrollBar = styled(ScrollBar)({
  flexGrow: 1,
});

const ProfileBox = styled("div")(({ theme }) => ({
  display: "flex",
  alignItems: "center",
  justifyContent: "space-between",
  padding: "12px 12px 12px 20px",
  color: theme.palette.primary.main,
  background: "#fafafa",
}));

const ChatStatus = styled("div")(({ theme }) => ({
  marginLeft: "12px",
  color: theme.palette.primary.main,
  "& h5": {
    marginTop: 0,
    fontSize: "14px",
    marginBottom: "3px",
  },
  "& span": { fontWeight: "500" },
}));

const ChatMessage = styled("div")(({ theme }) => ({
  padding: "8px",
  maxWidth: 240,
  fontSize: "14px",
  borderRadius: "4px",
  marginBottom: "8px",
  whiteSpace: "pre-wrap",
  wordBreak: "break-word",
  color: theme.palette.primary.main,
  background: "#fafafa",
}));

const MessageTime = styled("span")(({ theme }) => ({
  fontSize: "13px",
  fontWeight: "500",
  color: theme.palette.primary.main,
}));

const ChatImgContainer = styled("div")({
  padding: "20px",
  display: "flex",
  justifyContent: "flex-end",
});

const ChatImgBox = styled("div")(({ theme }) => ({
  padding: "8px",
  fontSize: "14px",
  maxWidth: 240,
  borderRadius: "4px",
  display: "flex",
  alignItems: "center",
  justifyContent: "flex-end",
  color: theme.palette.primary.main,
  background: "#fafafa",
}));

const ChatImg = styled("img")(() => ({ width: "40px" }));

// for previewing bot message
const globalMessageList: any[] = [];

export default function Chatbox({ togglePopup }: any) {
  const [isAlive, setIsAlive] = useState(false);
  const [message, setMessage] = useState("");
  const [messageList, setMessageList] = useState<
    {
      text: string;
      contactId: string;
      time?: string;
      id?: string;
      name?: string;
      avatar?: string;
      status?: string;
      mood?: string;
    }[]
  >([]);
  const currentUserId = "323sa680b3249760ea21rt47";
  const chatBottomRef = document.querySelector("#chat-scroll");

  const handleEmojiClick = (emojiObject: any) => {
    setMessage((message) => message + emojiObject.emoji);
  };

  const sendMessageOnEnter = (event: any) => {
    if (event.key === "Enter" && !event.shiftKey) {
      let tempMessage = message.trim();
      if (tempMessage !== "") {
        let tempList = [...messageList];
        let messageObject = {
          text: tempMessage,
          contactId: currentUserId,
        };
        tempList.push(messageObject);
        globalMessageList.push(messageObject);
        if (isAlive) setMessageList(tempList);

        if (isOpenEmojiPicker) openEmojiPicker();
        dummyReply();
      }
      setMessage("");
    }
  };

  const dummyReply = async () => {
    setTimeout(() => {
      let tempList = [...messageList];
      let messageObject = {
        text: "پیام خودکار پشتیبان",
        //contactId: "opponents contact id",
        contactId: "7863a6802ez0e277a0f98534",
        avatar: "/assets/images/customer-service.svg",
        name: "پشتیبانی آنلاین",
      };

      tempList.push(messageObject);
      globalMessageList.push(messageObject);
      if (isAlive) setMessageList(globalMessageList);
    }, 2000);
  };

  const scrollToBottom = useCallback(() => {
    if (chatBottomRef) {
      chatBottomRef.scrollTo({
        top: chatBottomRef.scrollHeight,
        behavior: "smooth",
      });
    }
  }, [chatBottomRef]);

  useEffect(() => {
    if (isAlive) {
      setMessageList([
        {
          contactId: "7863a6802ez0e277a0f98534",
          text: "سلام، چطور می توانم کمکتان کنم؟",
          time: "2018-02-10T08:45:28.291Z",
          id: "7863a6802ez0e277a0f98534",
          name: "پشتیبانی",
          avatar: "/assets/images/customer-service.svg",
          status: "online",
          mood: "",
        },
        // {
        //   contactId: "323sa680b3249760ea21rt47",
        //   text: "Yes",
        //   time: "2018-02-10T08:45:28.291Z",
        //   id: "323sa680b3249760ea21rt47",
        //   name: "Frank Powell",
        //   avatar: "/assets/images/faces/13.jpg",
        //   status: "online",
        //   mood: "",
        // },
        // {
        //   contactId: "7863a6802ez0e277a0f98534",
        //   text: "Don’t feel bad. It happens to a lot of us",
        //   time: "2018-02-10T08:45:28.291Z",
        //   id: "7863a6802ez0e277a0f98534",
        //   name: "John Doe",
        //   avatar: "/assets/images/face-1.jpg",
        //   status: "online",
        //   mood: "",
        // },
        // {
        //   contactId: "323sa680b3249760ea21rt47",
        //   text: "Do you ever find yourself falling into the “discount trap?”",
        //   time: "2018-02-10T08:45:28.291Z",
        //   id: "323sa680b3249760ea21rt47",
        //   name: "Frank Powell",
        //   avatar: "/assets/images/faces/13.jpg",
        //   status: "online",
        //   mood: "",
        // },
        // {
        //   contactId: "7863a6802ez0e277a0f98534",
        //   text: "Giving away your knowledge or product just to gain clients?",
        //   time: "2018-02-10T08:45:28.291Z",
        //   id: "7863a6802ez0e277a0f98534",
        //   name: "John Doe",
        //   avatar: "/assets/images/face-1.jpg",
        //   status: "online",
        //   mood: "",
        // },
        // {
        //   contactId: "323sa680b3249760ea21rt47",
        //   text: "Yes",
        //   time: "2018-02-10T08:45:28.291Z",
        //   id: "323sa680b3249760ea21rt47",
        //   name: "Frank Powell",
        //   avatar: "/assets/images/faces/13.jpg",
        //   status: "online",
        //   mood: "",
        // },
        // {
        //   contactId: "7863a6802ez0e277a0f98534",
        //   text: "Don’t feel bad. It happens to a lot of us",
        //   time: "2018-02-10T08:45:28.291Z",
        //   id: "7863a6802ez0e277a0f98534",
        //   name: "John Doe",
        //   avatar: "/assets/images/face-1.jpg",
        //   status: "online",
        //   mood: "",
        // },
      ]);
    }
  }, [isAlive]);

  useEffect(() => {
    scrollToBottom();
    return () => setIsAlive(true);
  }, [messageList, scrollToBottom]);

  const { palette } = useTheme();
  const primary = palette.primary.main;
  const textPrimary = palette.text.primary;

  const [isAttachedFile, setIsAttachedFile] = useState<boolean>(false);
  const [attachedFile, setAttachedFile] = useState<string>("");
  const [attachedFileSize, setAttachedFileSize] = useState<number>(0);
  const [attachedFileName, setAttachedFileName] = useState<string>("");

  const handleAttachmentClick = (event: any) => {
    console.log(event);
    const file = event.target.files[0];
    console.log(file);
    const reader = new FileReader();
    reader.readAsDataURL(file);

    reader.onloadend = function () {
      setAttachedFile(reader.result as string);
    };

    setAttachedFile(reader.result as string);
    setAttachedFileName(file.name);
    setAttachedFileSize(Math.round(file.size / 1024));
    setIsAttachedFile(true);
  };

  const [isOpenEmojiPicker, setIsOpenEmojiPicker] = useState<boolean>(false);

  const openEmojiPicker = () => {
    setIsOpenEmojiPicker(!isOpenEmojiPicker);
  };

  return (
    <ChatContainer>
      <ProfileBox>
        <Box display="flex" alignItems="center">
          <ChatAvatar
            src="/assets/images/customer-service.svg"
            status="online"
          />
          <ChatStatus>
            <H5>پشتیبانی آنلاین</H5>
            <Span>فعال</Span>
          </ChatStatus>
        </Box>
        <IconButton onClick={togglePopup}>
          <Clear fontSize="small" />
        </IconButton>
      </ProfileBox>
      <StyledScrollBar id="chat-scroll">
        {messageList.map((item, ind) => (
          <Box
            key={ind}
            p="20px"
            display="flex"
            sx={
              currentUserId === item.contactId
                ? { justifyContent: "flex-end" }
                : {}
            }
          >
            {currentUserId !== item.contactId && <Avatar src={item.avatar} />}
            <Box ml="12px">
              {currentUserId !== item.contactId && (
                <H5 mb={0.5} fontSize={14} color={primary}>
                  {item.name}
                </H5>
              )}
              <ChatMessage>{item.text}</ChatMessage>
              <MessageTime>1 دقیقه قبل</MessageTime>
            </Box>
          </Box>
        ))}

        {/* example of image sent by current user*/}
        {isAttachedFile && (
          <ChatImgContainer>
            <Box ml="12px">
              <ChatImgBox>
                <ChatImg
                  alt={attachedFileName}
                  //src="/assets/images/study-2.jpg"
                  src={attachedFile}
                />

                <Box ml="12px">
                  <H6 mt={0} mb={0.5}>
                    {attachedFileName}
                  </H6>
                  <MessageTime>{attachedFileSize + "KB"}</MessageTime>
                </Box>
              </ChatImgBox>
              <MessageTime>1 دقیقه قبل</MessageTime>
            </Box>
          </ChatImgContainer>
        )}
      </StyledScrollBar>

      <div>
        <Divider
          sx={{ background: `rgba(${convertHexToRGB(textPrimary)}, 0.15)` }}
        />

        <TextField
          multiline
          fullWidth
          inputProps={{ rowsMax: 4 }}
          value={message}
          placeholder="اینجا تایپ کنید ..."
          onKeyDown={sendMessageOnEnter}
          onChange={(e) => setMessage(e.target.value)}
          sx={{ "& textarea": { color: primary } }}
          InputProps={{
            endAdornment: (
              <Box display="flex">
                <IconButton onClick={openEmojiPicker} size="small">
                  <TagFaces />
                </IconButton>

                <IconButton size="small">
                  <label htmlFor="chat-attachment">
                    <Attachment />
                  </label>
                  <input
                    accept="image/*"
                    id="chat-attachment"
                    multiple
                    onChange={handleAttachmentClick}
                    type="file"
                    style={{ display: "none" }}
                  ></input>
                </IconButton>
              </Box>
            ),
            classes: { root: "pl-5 pr-3 py-3 text-body" },
          }}
        />
        <EmojiPicker
          open={isOpenEmojiPicker}
          searchPlaceHolder="ایموجی را سرچ کنید"
          onEmojiClick={handleEmojiClick}
        />
      </div>
    </ChatContainer>
  );
}
