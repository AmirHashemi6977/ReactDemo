...
export function getTimeDifference(date: Date) {
  ...
  if (difference < 60) return `${Math.floor(difference)} ثانیه`;
  else if (difference < 3600) return `${Math.floor(difference / 60)} دقیقه`;
  else if (difference < 86400) return `${Math.floor(difference / 3660)} ساعت`;
  else if (difference < 86400 * 30)
    return `${Math.floor(difference / 86400)} روز`;
  else if (difference < 86400 * 30 * 12)
    return `${Math.floor(difference / 86400 / 30)} ماه`;
  else return `${(difference / 86400 / 30 / 12).toFixed(1)} سال`;
}
